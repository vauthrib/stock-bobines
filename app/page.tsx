...
export default function Home() {
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-2 gap-6">
            {(isAdmin || isSuper) && (
              <button onClick={() => setCurrentPage('etat')} className="bg-green-600 hover:bg-green-700 text-white font-bold py-12 rounded-xl text-2xl shadow-lg transition transform hover:scale-105">📊 État</button>
            )}
            {isSuper && (
              <button onClick={() => setCurrentPage('autre')} className="bg-gray-600 hover:bg-gray-700 text-white font-bold py-12 rounded-xl text-2xl shadow-lg transition transform hover:scale-105">🔐 Autre</button>
            )}
            {!isSuper && !isAdmin && (
              <div className="col-span-2 text-center text-gray-500 py-12"><p className="text-lg">Bienvenue {currentUser}</p><p className="text-sm mt-2">Contactez un administrateur pour accéder aux fonctionnalités.</p></div>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div className="p-6">
              ...
              {autreSection === 'lots' && (
                <div>{lots.length===0 ? <p className="text-center py-8 text-gray-500">Cliquez sur l'onglet pour charger</p> : (<div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-gray-100"><tr><th className="px-3 py-2 text-left">Code</th><th className="px-3 py-2 text-left">Fournisseur</th><th className="px-3 py-2 text-left">Date</th><th className="px-3 py-2 text-right">Nb</th><th className="px-3 py-2 text-center">Action</th></tr></thead><tbody>{lots.map(l => (<tr key={l.id} className="border-b hover:bg-gray-50"><td className="px-3 py-2 font-mono font-semibold">{l.nom}</td><td className="px-3 py-2">{l.code_fournisseur}</td><td className="px-3 py-2">{new Date(l.date_reception).toLocaleDateString('fr-FR')}</td><td className="px-3 py-2 text-right">{l.nb_bobines}</td><td className="px-3 py-2 text-center"><button onClick={() => handleEditLot(l.id)} className="bg-orange-600 text-white px-3 py-1 rounded text-xs">✏️ Éditer</button></td></tr>))}</tbody></table></div>)}</div>
              )}
              {autreSection === 'users' && (
                <div>
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-md p-4 mb-4"><h3 className="font-semibold mb-3">➕ Ajouter</h3><div className="grid grid-cols-4 gap-2"><input placeholder="Login" value={newUser.login} onChange={e=>setNewUser({...newUser,login:e.target.value})} className="px-3 py-2 border rounded"/><input placeholder="Password" value={newUser.password} onChange={e=>setNewUser({...newUser,password:e.target.value})} className="px-3 py-2 border rounded"/><div className="flex items-center gap-2"><input type="checkbox" checked={newUser.isSuper} onChange={e=>setNewUser({...newUser,isSuper:e.target.checked})} className="w-4 h-4"/><label className="text-sm">Super</label></div><div className="flex items-center gap-2"><input type="checkbox" checked={newUser.isAdmin} onChange={e=>setNewUser({...newUser,isAdmin:e.target.checked})} className="w-4 h-4"/><label className="text-sm">Admin</label></div></div><button onClick={async ()=>{ try { const res=await fetch('/api/users',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(newUser)}); if(res.ok){setNewUser({login:'',password:'',isSuper:false,isAdmin:false});chargerUsers()} else alert((await res.json()).error) } catch(e){alert('❌')} }} className="mt-3 bg-green-600 text-white px-4 py-2 rounded-md text-sm">✓ Ajouter</button></div>
                  <div className="space-y-2">{users.map(u => (<div key={u.id} className="border rounded-md p-3 bg-gray-50"><div className="flex justify-between items-center"><div><span className="font-semibold">{u.login}</span><span className="ml-3 text-xs text-gray-500">{u.password}</span>{u.isSuper && <span className="ml-2 px-2 py-1 bg-purple-100 text-purple-800 rounded text-xs">Super</span>}{u.isAdmin && <span className="ml-2 px-2 py-1 bg-blue-100 text-blue-800 rounded text-xs">Admin</span>}{!u.actif && <span className="ml-2 px-2 py-1 bg-red-100 text-red-800 rounded text-xs">Désactivé</span>}</div><div className="flex gap-2"><button onClick={()=>setEditingUser(u)} className="bg-blue-600 text-white px-3 py-1 rounded text-xs">✏️</button><button onClick={async()=>{ if(confirm('Supprimer ?')){await fetch(`/api/users?id=${u.id}`,{method:'DELETE'});chargerUsers()}}} className="bg-red-600 text-white px-3 py-1 rounded text-xs">🗑️</button></div></div></div>))}</div>
                </div>
              )}
              {autreSection === 'history' && (<div>{history.length===0 ? <p className="text-center py-8 text-gray-500">Aucune connexion</p> : (<div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-gray-100"><tr><th className="px-3 py-2 text-left">Date</th><th className="px-3 py-2 text-left">User</th><th className="px-3 py-2 text-left">IP</th><th className="px-3 py-2 text-left">Appareil</th></tr></thead><tbody>{history.map(l => (<tr key={l.id} className="border-b hover:bg-gray-50"><td className="px-3 py-2 text-xs">{new Date(l.date_connexion).toLocaleString('fr-FR')}</td><td className="px-3 py-2 font-semibold">{l.utilisateur}</td><td className="px-3 py-2 text-xs text-gray-600">{l.ip_address||'-'}</td><td className="px-3 py-2 text-xs text-gray-600 max-w-xs truncate">{l.appareil}</td></tr>))}</tbody></table></div>)}</div>)}
              {autreSection === 'backup' && (<div className="space-y-4"><button onClick={handleExportBase} className="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-md font-semibold">📥 Exporter toute la base (CSV)</button><p className="text-xs text-gray-500">1 fichier CSV contenant : réceptions, bobines, mouvements, items</p><div className="bg-gray-50 border rounded-md p-4"><h3 className="text-sm font-semibold mb-3">📤 Importer</h3><input type="file" accept=".csv" onChange={e=>setBackupFile(e.target.files?.[0]||null)} className="flex-1 text-sm" />{backupFile && <p className="text-xs text-green-700 mt-2">✅ {backupFile.name}</p>}<button onClick={handleImportBase} className="mt-3 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md">📤 Restaurer</button><p className="text-xs text-red-600 mt-2">⚠️ Écrase les données actuelles.</p></div><div className="bg-blue-50 border rounded-md p-4"><h3 className="text-sm font-semibold mb-2">📊 Exports partiels</h3><div className="flex gap-2"><button onClick={()=>exporterCSV('stock')} className="bg-green-600 text-white px-4 py-2 rounded-md text-sm">💾 Stock</button><button onClick={()=>exporterCSV('mouvements')} className="bg-blue-600 text-white px-4 py-2 rounded-md text-sm">📥 Mouvements</button><input type="text" value={commandeFilter} onChange={e=>setCommandeFilter(e.target.value)} className="px-3 py-2 border rounded-md" placeholder="N° commande" /><button onClick={()=>exporterCSV('mouvements')} className="bg-blue-600 text-white px-4 py-2 rounded-md">Exporter</button></div></div><div className="bg-purple-50 border rounded-md p-4"><h3 className="text-sm font-semibold mb-2">📉 Consommation</h3><div className="flex gap-2 items-center"><input type="number" value={moisConso} onChange={e=>setMoisConso(parseInt(e.target.value)||1)} min="1" max="120" className="w-20 px-3 py-2 border rounded-md" /><span>mois</span><button onClick={()=>exporterCSV('consommation')} className="bg-orange-600 text-white px-4 py-2 rounded-md">📈 Exporter</button></div></div></div>)}
              {autreSection === 'reset' && (<div className="bg-red-50 border border-red-300 rounded-md p-6"><p className="text-sm text-red-800 mb-4">Supprime TOUTES les données (réceptions, bobines, mouvements, items). Les utilisateurs et l'historique sont conservés.</p><button onClick={handleReset} className="bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-6 rounded-md">⚠️ Réinitialiser</button></div>)}
            </div>
          </div>
        </div>
      </div>
  ...
  function ItemsManager({ onItemsChange }: { onItemsChange: () => void }) {
    ...
    useEffect(()=>{try{fetch(`/api/items?categorie=${cat}`).then(r=>r.json()).then(setItems)}catch(e){}},[cat])
    const add = async ()=>{if(!nouveau.trim())return;try{const r=await fetch('/api/items',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({categorie:cat,nom:nouveau.trim()})});if(r.ok){setNouveau('');try{fetch(`/api/items?categorie=${cat}`).then(r=>r.json()).then(setItems);onItemsChange()}catch(e){}}else alert((await r.json()).error)}catch(e){alert('❌')}}
    const upd = async (i:any,n:string)=>{if(!n.trim())return;try{const r=await fetch('/api/items',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:i.id,nom:n.trim()})});if(r.ok){setEdit(null);try{fetch(`/api/items?categorie=${cat}`).then(r=>r.json()).then(setItems);onItemsChange()}catch(e){}}else alert((await r.json()).error)}catch(e){alert('❌')}}
    const del = async (id:number)=>{if(!confirm('Supprimer ?'))return;try{await fetch(`/api/items?id=${id}`,{method:'DELETE'});try{fetch(`/api/items?categorie=${cat}`).then(r=>r.json()).then(setItems);onItemsChange()}catch(e){}}catch(e){alert('❌')}}
    const lab = {MATIERE:'Matières',DURETE:'Duretés',REVETEMENT:'Revêtements'}
    return (<div className="space-y-4"><div className="flex gap-2">{(['MATIERE','DURETE','REVETEMENT'] as const).map(c=>(<button key={c} onClick={()=>setCat(c)} className={`px-4 py-2 rounded-md ${cat===c?'bg-blue-600 text-white':'bg-gray-200'}`}>{lab[c]}</button>))}</div><div className="flex gap-2"><input value={nouveau} onChange={e=>setNouveau(e.target.value)} onKeyDown={e=>e.key==='Enter'&&add()} className="flex-1 px-4 py-2 border rounded-md" placeholder={`Nouveau ${lab[cat].toLowerCase()}...`}/><button onClick={add} className="bg-green-600 text-white px-4 py-2 rounded-md">➕ Ajouter</button></div><div className="space-y-2 max-h-96 overflow-y-auto">{items.length===0?<p className="text-center py-4 text-gray-500">Aucun</p>:items.map(i=>(<div key={i.id} className="flex items-center gap-2 p-2 border rounded-md">{edit?.id===i.id?(<><input value={edit.nom} onChange={e=>setEdit({...edit,nom:e.target.value})} onKeyDown={e=>e.key==='Enter'&&upd(i,edit.nom)} className="flex-1 px-3 py-1 border rounded-md" autoFocus/><button onClick={()=>upd(i,edit.nom)} className="bg-green-600 text-white px-3 py-1 rounded text-sm">✓</button><button onClick={()=>setEdit(null)} className="bg-gray-400 text-white px-3 py-1 rounded text-sm">✕</button></>):(<><span className="flex-1 font-medium">{i.nom}</span><button onClick={()=>setEdit({...i})} className="bg-blue-600 text-white px-3 py-1 rounded text-sm">✏️</button><button onClick={()=>del(i.id)} className="bg-red-600 text-white px-3 py-1 rounded text-sm">🗑️</button></>)}</div>))}</div></div>)
  }
}